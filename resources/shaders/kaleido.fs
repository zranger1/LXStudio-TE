...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// --- Kaleidoscope ---
	uv = mix( uv, Kaleidoscope( uv, NUM_SIDES, vTime*10. ), USE_KALEIDOSCOPE );
	
	uv *= ZOOM_FACTOR+(BASS_INTENSITY)* (texture(iChannel0, vec2(.01,0)).r);
	...
	for ( int i = 0; i < 128; i++ ) {
		...
		float velintensity = texture(iChannel0, vec2(1.0+uv.x,0)).r;
		...
		//sound loudness to intensity
		float intensity = texture(iChannel0, vec2(pos.y,0)).r;
